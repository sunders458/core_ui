...
<main class="main-content w-full px-[var(--margin-x)] pb-8">
    ...
    <div
      class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-6 xl:grid-cols-3 "
    >
      ...
      <div class="flex flex-col">
        ...
        <div class="flex space-x-2">
            ...
            class="badge rounded-full 
              @if ($item->type_logement=='BEVERLY')
              bg-info 
              @endif
              @if ($item->type_logement=='EXCELLIUM')
              bg-warning 
              @endif
              @if ($item->type_logement=='HARMONIE')
              bg-secondary 
              @endif
              @if ($item->type_logement=='MEYERS C')
              bg-error 
              @endif
              @if ($item->type_logement=='SOMOLI')
              bg-success
              @endif
            ...
            >
            <p>{{$item->superficie}} M<sup>2</sup></p>
            <div
                class="mx-2 my-1 w-px self-stretch bg-slate-200 dark:bg-navy-500"
            ></div>
            <p>{{$item->nbre_pieces}} Pièces</p>
            </div>
        </div>
        <div class="mt-2">
          <a
          href="{{route('logements.show',[$item->id])}}"
          ...
          {{$item->ilot}}
            ...
            <div class="grow space-y-1">
              <div class="flex justify-between">
                <p class="font-medium">Taux d'avancement</p>


                <p class="
                  @if ($item->etat_avancement ==0)
                  text-transparent
                  @else
                  text-success
                  @endif
                ">
                  {{$item->etat_avancement}}%
                </p>
              </div>
              <div class="progress h-1.5 bg-slate-150 dark:bg-navy-500">
                <div class="
                @if ($item->etat_avancement ==0)
                  w-0/12 rounded-full bg-warning
                @endif
                @if ($item->etat_avancement >0 && $item->etat_avancement <=10 )
                  w-1/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >10 && $item->etat_avancement <=20 )
                  w-2/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >20 && $item->etat_avancement <=30 )
                  w-3/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >30 && $item->etat_avancement <=40 )
                  w-4/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >40 && $item->etat_avancement <=50 )
                  w-5/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >50 && $item->etat_avancement <=60 )
                  w-6/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >60 && $item->etat_avancement <=70 )
                  w-7/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >70 && $item->etat_avancement <=80 )
                  w-8/12 rounded-full bg-success
                @endif

                @if ($item->etat_avancement >80 && $item->etat_avancement <=90 )
                  w-9/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement >90 && $item->etat_avancement <100 )
                  w-10/12 rounded-full bg-success
                @endif
                @if ($item->etat_avancement ==100 )
